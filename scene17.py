...
def remake_node(old_node: Mobject, *text: str, color = BLUE_C, cr=0.07) -> Mobject:
    """
    ...
    cr : float
        (default:0.07, optionnal) corner radius of the rounded rectangle behind the text
    ...
    N = LabeledNode(Paragraph(*text,
                              fill_opacity=0.,
                              alignment="center",
                              font_size=15, # copy font size
                              color=BLACK),
                    cr=cr)\
        .move_to(old_node)\
        .set_z_index(old_node.z_index, True) #copy position and z_index
    N[1].set(fill_color=color, color=color, corner_radius=cr) # set color, z_index and corner radius for the rectangle
...
class Scene17(MovingCameraScene):
    """
    this scene shows the rules that will limit the expansion of a decision tree
    """
    def construct(self):
        # create tree
        A = TreeA()
        ...
        D = Group(*[Dot(color=colours[i], z_index=Z_index[i], radius=0.05) for i in range(len(colours))]).arrange_in_grid(4, 16, buff=0.05)\
            .next_to(A[0], UP)\
        ...
        # camera setup
        self.camera.frame.move_to(Group(D, A[0])).match_width(Group(D, A[0])).scale(1.2)

        # initial creation
        self.play(FadeIn(D), FadeIn(R))
        self.wait()
        self.play(DrawBorderThenFill(A[0][1]))

        # position the dots randomnly inside the first node
        P0 = random_placement(D, A[0])
        self.play(AnimationGroup(*[d.animate.move_to(p) for (d,p) in zip(D,P0)]+[FadeOut(R)], group=Group(D,R), lag_ratio=0.01))
        self.wait()
        ...
        N = [6,8,10,12] # nodes recieving dots
        ...
        # highlight the node that is pure
        self.play(Flash(A[8][0], color=GOLD, stroke_width=0.2, buff=1))
        self.wait()

        """LEVEL 3"""
        # its the same thing but with more branches

        N = [14,16,18,20,22,24] # nodes recieving dots
        # distribution of the dots
        separation = {14: [i for i in range(9)] + [i for i in range(37, 43)],
                      16: [i for i in range(43, 52)],
                      18: [i for i in range(15, 21)] + [i for i in range(52, 57)],
                      20: [i for i in range(21, 31)],
                      22: [i for i in range(57, 64)],
                      24: [i for i in range(31, 37)]}
        ...
        # change camera view
        view = Group(*A[:25])
        self.play(self.camera.frame.animate.move_to(view).match_width(view).scale(1.1))
        ...
        """LEVEL 4"""
        # its the same thing but the only nodes left are pure (=leaf)

        N = [26,28,30,32] # nodes recieving dots
        # distribution of the dots
        separation = {26: [i for i in range(9)],
                      28: [i for i in range(37, 43)],
                      30: [i for i in range(15, 21)],
                      32: [i for i in range(52, 57)]}
        ...
        # change camera view
        self.play(self.camera.frame.animate.move_to(A).match_width(A).scale(1.1))
